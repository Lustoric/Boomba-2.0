...
def loadSongs():
    global song_data, filtered_data
    song_data.clear()
    for root, dirs, files in os.walk(rootpath):
        for filename in fnmatch.filter(files, pattern):
            ...
            try:
                ...
                genre = str(genre) if genre else "Unknown Genre"
                song_data.append({
                    "filename": filename,
                    "path": path,
                    "artist": artist,
    ...
    displaySongs(filtered_data)

def displaySongs(song_list):
    playlistBox.configure(state="normal")
    playlistBox.delete("0.0", "end")
    for i, song in enumerate(song_list):
        playlistBox.insert("end", f"{song['filename']} - {song['artist']} - {song['genre']}\n")
...
def select(index=None):
    ...
    if index is not None:
        selectedIndex = index
    song = filtered_data[selectedIndex]
    songLabel.configure(text=song["filename"])
    try:
        ...
        pauseButton.configure(state="normal", text="Pause")
        updateProgressBar()
    except Exception as e:
        songLabel.configure(text=f"Error: {e}")
...
def playNext():
    ...
    selectedIndex = (selectedIndex + 1) % len(filtered_data)
...
def highlightSong(index):
    ...
    displaySongs(filtered_data)
    playlistBox.configure(state="normal")
    lines = playlistBox.get("0.0", "end-1c").split("\n")
    lines[selectedIndex] = "> " + lines[selectedIndex]
    playlistBox.delete("0.0", "end")
    for line in lines:
        playlistBox.insert("end", line + "\n")
    playlistBox.configure(state="disabled")

def searchSongs():
    query = searchEntry.get().lower()
    filtered = [s for s in song_data if query in s["artist"].lower() or query in s["genre"].lower()]
...
searchFrame = ctk.CTkFrame(app, fg_color="transparent")
searchFrame.pack(pady=10)

searchEntry = ctk.CTkEntry(searchFrame, placeholder_text="Search by artist or genre", width=300)
searchEntry.pack(side="left", padx=5)

searchButton = ctk.CTkButton(searchFrame, text="Search", command=searchSongs)
searchButton.pack(side="left", padx=5)

playlistBox = ctk.CTkTextbox(app, height=200, width=800, font=("Segoe UI", 14))
...
pauseButton = ctk.CTkButton(btnFrame, text="Pause", command=pauseSong, width=80, state="disabled")
...
if filtered_data:
    ...
    songLabel.configure(text="No songs found.")
